...
async def decompose_description(description, block_summaries, original_text=None, user_instruction=None, retry_feedback=None):
    ...
    if original_text and retry_feedback:
        ...
        prompt = f"""
            ...
            Please revise the instructions accordingly. If needed, add more steps. Keep the same structure.
...
async def generate_agent_json_from_subtasks(instructions_text, blocks_json):
    ...
    is_valid, error = validator.validate(agent_json, blocks_json) # type: ignore
    if not is_valid:
        return None, error

    # valid, message = validate_agent_json(agent_json)
    # if not valid:
    #     return None, message
...
async def update_decomposition_incrementally(improvement_request: str, current_instructions: str, block_summaries: list, original_updated_instructions: str = None, validation_error: str = None) -> str:
    ...
    if original_updated_instructions and validation_error:
        ...
        Please revise the instructions accordingly. If needed, add more steps or fix the issues. Keep the same structure and preserve unchanged parts.
...
async def update_agent_json_incrementally(updated_instructions: str, current_agent_json: dict, blocks_json: list) -> tuple:
    ...
    try:
        ...
        logging.error(f"Error updating agent JSON: {e}")
